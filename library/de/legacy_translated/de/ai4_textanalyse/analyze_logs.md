...
language: "de"
source_path: "de/ai4_textanalyse/analyze_logs.md"
...
# Task
You are given a log file from one of the company's servers. The log file contains entries of various events and activities. Your task is to analyze the log file, identifizieren patterns, anomalies, and potential issues, and provide insights into the reliability and performance of the server based on the log data.

# Actions
- **Analyze the Log File**: Thoroughly examine the log entries to identifizieren any unusual patterns or anomalies that could indicate potential issues.
...
- **Identifizieren Recurring Issues**: Look for any recurring patterns or persistent issues in the log data that could potentially impact server reliability.
...
# Restrictions
- **Avoid Irrelevant Informationen**: Do not include details that are not derived from the log file.
- **Base Assumptions on Data**: Stelle sicher that all assumptions about the log data are clearly supported by the Informationen contained within.
- **Focus on Data-Driven Advice**: Gib specific recommendations that are directly based on your analysis of the log data.
- **Exclude Personal Opinions**: Refrain from including subjective Bewertungen or personal opinions in your analysis.
